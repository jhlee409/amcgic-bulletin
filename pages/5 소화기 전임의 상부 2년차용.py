...
if st.session_state.get('logged_in', False):  # 로그인 상태 확인
    ...
    with tab1:
        ...
        st.markdown(
            ...
            1. [AMC GI 상부 Education Program](https://amcgic-training.streamlit.app/)에 올려져 있는 교육 자료들은 1년차 뿐 아니라 여러분에게도 매우 도움이 되는 내용들이니 꼭 보기 바랍니다.
            1. AI patient Hx. taking 훈련 프로그램은 매우 강력한 프로그램입니다. 완전히 자기 것이 될 정도로 훈련하면 실력향상에 큰 도움이 되니 열심히 하세요.
    ...
    with tab2:
        ...
        with st.expander("SSIT 내용을 보려면 여기를 눌러주세요"):
            st.markdown(
                ...
                1. EUS scope를 blind 방식으로, simulator UES에 삽입하는데, 경고음이 안나도록 통과하는 것이 목표입니다. 두 번 시도해서 한 번 성공하면 통과입니다.
        ...
        with st.expander("바람직한 EUS, ESD training 요령의 내용을 보려면 여기를 눌러주세요"):
            st.markdown(
                '''
                1. 근무 전반부는 내시경 로딩이 많아 ESD, EUS 세션 참여가 어려울 겁니다. 조금 여유가 나면 EUS, ESD 세션에 들어가서 교수님의 지시에 따르고 출석 사인 받으세요. 나중에 일괄적으로 걷어 기록합니다.
                ...
                1. 또한 [AMC GI 상부 Education Program](https://amcgic-training.streamlit.app/)에 올려져 있는 Dx. EUS 강의 3개도 반드시 시청하기 바랍니다.
                1. GI training program는 출석 체크가 되므로, 끝낼 때는 반드시 왼쪽 아래 로그아웃 버튼을 눌러 종결하세요. 그냥 나가면 출석처리가 안됩니다.
                1. 다시 한 번 강조하지만 SSIT 통과 전에는 EUS scope를 환자에게 UES를 직접 삽입하는 것은 금지입니다. 교수님이 하라고 해도 SSIT 통과 전이면 사정을 말씀드리고 하지 마세요. 물론 다른 과정은 감독하에 해도 됩니다.
